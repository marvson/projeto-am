...
for j in range(5):
    ...
    for train, test in kfold.split(x,y):
        ...
        if j==1: ytest,ypred, a, b = knn(classes,x[train],y[train],x[test],y[test])
        if j==2: ytest,ypred, a, b = parzen(classes,x[train],y[train],x[test],y[test])
        ...
        fsco.append(fsc)
        ypreds.append(ypred)
        plt.title ("Validação "+nome[j])
        # plt.xlabel('valor de h')
        # plt.ylabel('score')
        plt.plot (a,b,label='fold'+str(f))
        plt.legend()
